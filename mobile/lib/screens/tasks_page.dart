...
class _TasksPageState extends State<TasksPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ZenexGlowBackground(safeArea: false, child: FutureBuilder<List<Map<String, dynamic>>>(
        future: future,
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return _ErrorState(message: friendlyError(snapshot.error!), onRetry: _reload);
          }
                    ...
                    itemBuilder: (_, index) {
                      ...
                      return ChoiceChip(
                        ...
                        labelStyle: TextStyle(
                          color: selected ? zenexPrimaryText(context) : zenexMutedText(context),
            ...
            child: Column(
              ...
              children: [
                Text(
                  ...
                  style: TextStyle(color: zenexPrimaryText(context), fontSize: 19, fontWeight: FontWeight.w900),
                ),
                const SizedBox(height: 5),
                Text(
                  '$total published ${total == 1 ? 'task' : 'tasks'} ready to explore.',
                  style: TextStyle(color: zenexMutedText(context), height: 1.3),
                ),
              ],
            ),
          ),
            ...
            children: [
              ...
              Align(
                ...
                child: Text(
                  ...
                  maxLines: 2,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(color: zenexMutedText(context), height: 1.4, fontSize: 13),
                ),
              ),
              const SizedBox(height: 13),
              Row(
                children: [
                  ...
                  const SizedBox(width: 14),
                  _Meta(icon: proof ? Icons.verified_outlined : Icons.flash_on_rounded, text: proof ? 'Proof required' : 'Quick task'),
                  const Spacer(),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 13, vertical: 8),
                    decoration: BoxDecoration(gradient: const LinearGradient(colors: [kBlue, kPurple]), borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: kPurple.withOpacity(.18), blurRadius: 12)]),
...
class _Meta extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
        ...
        children: [
          Icon(icon, size: 15, color: zenexMutedText(context)),
          const SizedBox(width: 5),
          Text(text, style: TextStyle(color: zenexMutedText(context), fontSize: 11, fontWeight: FontWeight.w700)),
        ],
      );
}
...
class _EmptyTasks extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(
          children: [
            Icon(Icons.inbox_rounded, size: 42, color: zenexMutedText(context)),
            SizedBox(height: 12),
            Text('No tasks in this category', style: TextStyle(fontWeight: FontWeight.w900)),
            SizedBox(height: 5),
            Text('Try another filter or refresh the task list.', textAlign: TextAlign.center, style: TextStyle(color: zenexMutedText(context))),
          ],
        ),
      );
}
  ...
  Widget build(BuildContext context) => Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Icon(Icons.cloud_off_rounded, size: 48, color: zenexMutedText(context)),
              const SizedBox(height: 12),
              const Text('Could not load tasks', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
              const SizedBox(height: 7),
              Text(message, textAlign: TextAlign.center, style: TextStyle(color: zenexMutedText(context))),
              const SizedBox(height: 16),
