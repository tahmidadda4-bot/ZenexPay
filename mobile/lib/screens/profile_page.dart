...
class _ProfilePageState extends State<ProfilePage> {
  ...
  @override void initState() {
    super.initState();
    profileFuture = SupabaseService.profile();
  }
      ...
      body: ZenexGlowBackground(safeArea: false, child: ListView(
        ...
        children: [
          ...
          FutureBuilder<Map<String, dynamic>?>(
            ...
            builder: (_, snapshot) {
              final numericId = '${snapshot.data?['user_numeric_id'] ?? ''}'.trim();
              return Card(
                child: ListTile(
                  leading: const Icon(Icons.badge_rounded),
                  title: const Text('UID', style: TextStyle(fontWeight: FontWeight.w900)),
                  subtitle: Text(
                    numericId.isEmpty ? 'Generating UID…' : numericId,
                    style: const TextStyle(fontWeight: FontWeight.w800, letterSpacing: 1.0),
                  ),
                  trailing: IconButton(
                    tooltip: 'Copy UID',
                    onPressed: numericId.isEmpty ? null : () async {
                      await Clipboard.setData(ClipboardData(text: numericId));
