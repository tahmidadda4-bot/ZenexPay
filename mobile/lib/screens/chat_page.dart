...
class _ChatPageState extends State<ChatPage> {
  ...
  bool loading = true, sending = false, aiActive = true, transferred = false;
  ...
  Future<void> _init() async {
    try {
      ...
      if (aiState?['status'] == 'admin_handoff' && mounted) {
        aiActive = false;
        transferred = true;
      }
      await _load();
  ...
  Future<void> _send() async {
    ...
    try {
      if (aiActive && !transferred) {
        final result = await SupabaseService.aiSupportTurn(conversationId: conversationId!, message: text);
        controller.clear();
        if (result['handoff'] == true) {
          if (mounted) {
            setState(() { aiActive = false; transferred = true; });
            _msg('I am transferring this conversation to a ZenexPay Admin.');
          }
        }
        await _load();
      } else {
      ...
      // If the AI service is unavailable, preserve the original support flow
      // and hand the message to the human-admin channel instead of blocking the user.
      try {
        await SupabaseService.sendSupportMessage(conversationId: conversationId!, message: text);
        controller.clear();
        if (mounted) {
          setState(() { aiActive = false; transferred = true; });
          _msg('AI support is temporarily unavailable. Your message was sent to an Admin.');
        }
        await _load();
      } catch (fallbackError) {
        if (mounted) _msg(friendlyError(fallbackError));
      }
    }
  ...
  Widget build(BuildContext c) => Scaffold(
        appBar: AppBar(
          title: Row(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('ZenexPay Support', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900)), Text(transferred ? 'Admin is helping you' : 'AI Support • Admin when needed', style: const TextStyle(fontSize: 10, color: Colors.grey))]),
          ]),
        ...
        body: loading
            ...
            : Column(children: [
                Container(margin: const EdgeInsets.fromLTRB(14, 8, 14, 4), padding: const EdgeInsets.all(13), decoration: BoxDecoration(color: Theme.of(c).colorScheme.primary.withOpacity(.065), borderRadius: BorderRadius.circular(18)), child: Row(children: [Icon(transferred ? Icons.support_agent_rounded : Icons.auto_awesome_rounded, size: 19), const SizedBox(width: 8), Expanded(child: Text(transferred ? 'Your conversation has been transferred to a ZenexPay Admin.' : 'AI Support will guide you first. If it cannot solve the issue, your conversation will be transferred to an Admin.', style: const TextStyle(fontSize: 12.5, height: 1.35)))])),
                Expanded(
