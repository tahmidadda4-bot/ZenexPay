...
class _HomePageState extends State<HomePage> {
  static bool _onboardingShownThisRun = false;
  ...
  void initState() {
    ...
    // Show the app tour once per app run after the main shell is ready.
    if (!_onboardingShownThisRun) {
      _onboardingShownThisRun = true;
  ...
  void goTo(int value) {
    if (!mounted || value < 0 || value > 4) return;
    setState(() => index = value);
  }

  Future<bool> _handleBack() async {
    // Never let the Android back button close the app from the home shell.
    // If the user is on another bottom tab, go back to Home first.
    if (index != 0) {
      setState(() => index = 0);
      return false;
    }
  ...
  Widget build(BuildContext context) {
    final pages = <Widget>[
      DashboardPage(onTabSelected: goTo),
      const TasksPage(),
